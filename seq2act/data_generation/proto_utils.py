    ...
    if padding_shape is None:
        ...
        for key in ui_object_attributes:
            if key == 'obj_str_seq':
                ...
                merged_features['ui_obj_' +
                                key] = ui_object_attributes[key].copy()
...
def get_view_hierarchy_leaf_relation(view_hierarchy_leaf_nodes):
    ...
    horizontal_adjacency = np.zeros(
        (vh_node_num, vh_node_num), dtype=np.float32)
...
def padding_dictionary(orig_dict, padding_shape_dict, padding_type_dict,
    ...
    for key in orig_dict:
        if padding_shape_dict[key]:
            ...
            padded_dict[key] = np.array(
                orig_dict[key], dtype=padding_type_dict[key])
    return padded_dict
...
def features_to_tf_example(features):
    ...
    for k, v in features.items():
        if not isinstance(v, np.ndarray):
            raise ValueError(
                'Value field: %s is not numpy array' % str((k, v)))
        v = v.flatten()
        if np.issubdtype(v.dtype.type, np.string_):
            new_features[k] = tf.train.Feature(
